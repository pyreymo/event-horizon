...
render-thread WorldView修正后单面仍闪烁，因此framework/render采样时序不是根因。两次实际draw capture均为`Draws=18`，对应三个已执行批次各一条Opaque indexed、三条辅助indexed和两条辅助non-indexed；不存在重复主视图Opaque draw。第四次builder发生后framework立即完成capture，而其commands尚未消费，故只看到三批是正常producer/consumer差，不是漏提交。当前窄capture扩充Opaque逐条状态：VS/PS/input layout、VS/PS constant buffer身份和首个可读hash、全部PS SRV view/resource身份。下一次采集用这三条draw判断frame间shader permutation、材质constant或纹理绑定是否变化；仍不恢复通用executor tracer。

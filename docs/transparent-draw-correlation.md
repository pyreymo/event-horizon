...
方向随后再次收紧：上述soak只会证明不可见command可重复执行，不能替代肉眼或像素证据，因此暂不要求采集。旧 `TransparentDrawCorrelationTracer` 与 `MaterialSubmissionContainerProbe` 已从运行时代码删除，相关按钮和大段draw/command日志也已移除。新 `NativeOpaquePreviewController` 只维护一个camera-facing persistent quad，geometry同时包含正反绕序并提供完整UV。首次点击得到 `Unavailable: no active world camera`；原因是framework线程上的`Scene.CameraManager.CurrentCamera`并不是已确认render现场的可靠入口，预览根本没有创建实例或调用builder。修正版将camera-facing transform移到Underpaint的render rendezvous：按实际view/subview从`Render.Manager.Views[].SubViews[].Camera`取得本次`RenderCamera`，用其逆View矩阵生成world并以同一View×Projection做clip检查，随后才写入owned current/previous World CB并提交。UI区分`Waiting`与builder已经接受的`Submitted`。下一次实机的唯一问题是该原生quad能否实际显示；若已`Submitted`仍不可见，剩余范围只在material discard、depth/GBuffer输出和vertex semantic，不再混入提交链路或生命周期工程化。

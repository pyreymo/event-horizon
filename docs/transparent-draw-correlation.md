...
逐条Opaque状态确认VS、PS、input layout、材质主纹理SRV 0–2及其他材质资源均稳定；变化只出现在transform/global frame constants和一个游戏场景公共SRV ring，故selector/material/context恢复和command资源生命周期不是当前闪烁来源。IDA对游戏原生non-skinned updater `0x266DD0`给出决定性证据：WorldView使用`Render.Manager.Views[30].SubViews[12].Camera.ViewMatrix`。Context hook看到的`CurrentSubViewIndex=11`只是builder rendezvous标签，不能直接作为`Render.View.SubViews[]`索引；而`Control.CameraManager` active scene camera也不是shader实际使用的render camera。当前后端在同一render rendezvous中改读原生View 30/SubView 12 camera，再计算FixedWorld×View和current/previous CB。该错误会影响正式持续后端，因此必须在当前阶段关闭；窄capture暂保留到最终实机验收。

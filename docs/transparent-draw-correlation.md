...
方向随后再次收紧：上述soak只会证明不可见command可重复执行，不能替代肉眼或像素证据，因此暂不要求采集。旧 `TransparentDrawCorrelationTracer` 与 `MaterialSubmissionContainerProbe` 已从运行时代码删除，相关按钮和大段draw/command日志也已移除。新 `NativeOpaquePreviewController` 只维护一个camera-facing persistent quad，geometry同时包含正反绕序并提供完整UV。首次点击得到 `Unavailable: no active world camera`，第二版改从`Render.Manager.Views[].SubViews[].Camera`读取后又持续停在`Waiting`；后者的错误是把builder日志中的view/subview标签当作这两个数组的直接索引。固定view-space Z `+5`版移除了等待并实机进入`Submitted`，但仍无可见像素，说明command成立而屏幕位置尚未闭环。复查本仓库旧可见预览确认正确入口是`Control.Instance()->CameraManager.GetActiveCamera()`，并且其`SceneCamera.ScreenPointToRay`已经产生过肉眼可见的屏幕中心quad。当前版本复用同一中心/邻近射线构造近距离World和right/up基，逐帧乘`SceneCamera.ViewMatrix`后写入owned current/previous WorldView。下一次实机只验证这个有既有可见基准的位置；若仍不可见，剩余范围才收敛到material discard/GBuffer输出和vertex semantic。

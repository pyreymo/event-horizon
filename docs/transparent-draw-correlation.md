...
逐条Opaque状态确认VS、PS、input layout、材质主纹理SRV 0–2及其他材质资源均稳定；变化只出现在transform/global frame constants和一个游戏场景公共SRV ring，故selector/material/context恢复和command资源生命周期不是当前闪烁来源。IDA对游戏原生non-skinned updater `0x266DD0`给出决定性证据：WorldView使用`Render.Manager.Views[30].SubViews[12].Camera.ViewMatrix`。Context hook看到的`CurrentSubViewIndex=11`只是builder rendezvous标签，不能直接作为`Render.View.SubViews[]`索引。

使用原生render camera后闪烁仍存在，因此增加了仅覆盖四次submission的World CB关联。结果为builder前后hash不变，实际Opaque Draw的`VSCB1`完整hash与提交端一致，current/previous两半也一致；CB复用、延迟覆盖和builder副作用均被排除。决定性异常是矩阵`M14/M24/M34/M44`为反规格化垃圾值。FFCS确认`Render.Camera.ViewMatrix`物理上是64-byte `Matrix4x4`；IDA确认它在此路径上的真实语义是带隐式齐次列的3x4 affine storage：`0x338870`不读取索引3/7/11/15，`0x266DD0`打包World CB时显式写入`(0,0,0,1)`。托管普通4x4乘法此前错误消费了未初始化slot。后端现已在FixedWorld×View前规范化该列，与原生乘法语义一致；窄capture保留到这次修复的实机验收。

齐次列修复后日志确认矩阵、World CB和实际Draw均正确稳定，但肉眼闪烁未消失，因此transform/CB分支关闭。直接从游戏SqPack读取目标材质对应的`c0101e0378_top.mdl`后，确认mesh 0使用相同的stream stride `20/24`和Position/BlendWeights/BlendIndices + Normal/Tangent/Color/UV declaration；问题在owned payload而非declaration。此前weights为`FF-FF-FF-FF`、normal half4 W为1、packed bitangent为`FF-FF-FF-FF`、UV1为`(0,1)`，均不符合目标资源。平面已改为单骨权重、normal `(0,0,1,0)`、与平面UV方向一致的packed bitangent、white color和该材质模型使用的UV1 `(-1,2)` sentinel；实机仍闪。

随后对实际Opaque draw的输出状态做了只读比较。连续三帧的五个RTV及其resource/format/size、DSV、viewport、blend state和所有MRT write mask、depth/stencil state与reference、rasterizer及topology均保持一致；shader、layout、材质SRV和World CB也继续稳定。fixed pipeline状态因此不是当前帧间交替的来源。

曾尝试在Draw hook中把仍绑定的RTV区域复制到staging texture并立刻`MapSubresource`读取，以比较实际G-buffer输出。该实现把GPU完成等待引入原生render command处理链，实机直接硬卡死并产生`E0434352`崩溃；调用栈位于`ffxiv_dx11.exe+0x23424A/+0x234815/+0x233D31`的ImmediateContext命令处理现场。该探针已完整回退，不得再次采用“当前draw后同步Copy+Map”的方式。若后续确需像素级关联，只允许使用带明确GPU完成边界的异步readback，或RenderDoc等外部帧捕获；在此之前优先验证当前角色衣服材质是否本就不适合作为rigid静态几何的正式测试材质。

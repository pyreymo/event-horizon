...
屏幕射线版本随后首次显示出带纹理的原生材质图案，证明owned geometry、material、WorldView和Opaque像素输出已经闭环。固定World、缩小尺寸与单层6-index正面也已实机通过，但单面仍闪烁。干净日志显示11.9秒内1848次builder submission，约155次/秒，与逐帧一次一致。当前版本把WorldView所有权进一步纠正：framework线程只在Show时创建不可变FixedWorld；render rendezvous在同一线程读取当前active camera View、计算`FixedWorld * View`、写current/previous CB并调用builder，避免两个线程使用不同相机时刻。实际draw统计直接复用仍存在的D3D owned VB/IB过滤capture，自动收集前四次submission并输出`Pass/DrawType/Count`分组，不恢复通用executor tracer。下一次实机同时验证闪烁是否消失和每次builder实际生成的主/辅助draw规模。

...
但command执行并不等于可见像素，且此前实机从未肉眼看到原生三角形，因此持续soak暂缓。当前构建删除旧透明capture、native duplicate、一次性triangle和soak UI，以及对应的container/consumer/command日志控制器。唯一保留的debug入口是 `Show native opaque preview`：它创建带0..1 UV的双面quad。第一次实现从framework线程读取了错误的scene camera入口；第二次又把builder的view/subview标签误作`Render.Manager`数组索引，因而持续显示`Waiting for the main-view render camera`。本仓库旧预览实际使用`Control.CameraManager.GetActiveCamera()`，但这里无需恢复该依赖：non-skinned constant的真实输入是已乘相机的`WorldView`，此前命令闭环也已验证单位矩阵加view-space Z `+5`。当前实现直接以该固定view-space矩阵创建持续实例，点击后只等待原生render rendezvous，builder实际接受后UI显示`Submitted`。该预览通过肉眼验收后才恢复持续实例和生命周期验证。

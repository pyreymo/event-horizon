...
逐条Opaque状态显示VS、PS、layout、材质主纹理SRV 0–2和其余材质资源均稳定；仅VS transform/global常量随帧变化，PS全局frame buffer和一个场景公共SRV按游戏ring正常轮换。这排除了selector、material texture、重复draw和command资源悬空。IDA确认原生non-skinned transform updater `0x266DD0`从`Render.Manager.Views[30].SubViews[12].Camera.ViewMatrix`生成WorldView；hook的`View=30/SubView=11`只是Context调度标签。

后续窄capture把提交端World CB与实际Opaque Draw的`VSCB1`完整128 bytes关联起来：builder前后、三个连续Draw以及current/previous两半hash全部一致，排除了builder修改、持久CB延迟覆盖和history交替。矩阵内容同时暴露出第四列为反规格化垃圾值。FFCS确认字段物理类型仍是64-byte `Matrix4x4`；IDA则证明语义不是普通4x4：`0x338870`仅使用3x4 affine payload，忽略float索引3/7/11/15，`0x266DD0`转置写入World CB时再显式补齐`(0,0,0,1)`。托管`Matrix4x4`乘法此前错误读取了四个未初始化slot。当前修复在乘法前将`M14/M24/M34/M44`规范化为`0/0/0/1`，与原生乘法和CB打包一致。`Clear EventHorizon logs`和四submission窄capture暂留到本次修复实机确认后收尾。

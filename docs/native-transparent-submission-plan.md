...
> 2026-07-19 状态：原生 pass builder `ffxiv_dx11.exe+0x283320` 已证实可消费Underpaint-owned geometry、显式加载的不透明Material、non-skinned current/previous World CB、owned `g_InstanceParameter`和owned shader selection。source geometry/material/Model/SHPK selection/角色实例常量及pass/view mask均已脱离；canonical scene keys、受限mask和最小Model facade均已实机通过。逐帧Opaque capture确认shader、layout和材质主纹理稳定，也没有重复主视图draw；闪烁最终定位为WorldView使用了错误相机。IDA确认游戏原生non-skinned updater `0x266DD0`固定读取`Render.Manager.Views[30].SubViews[12].Camera.ViewMatrix`，而builder rendezvous的`CurrentSubViewIndex=11`不是该数组索引。后端现已严格复用原生transform camera，不再使用Control active camera。清理日志和窄capture暂保留用于最终实机关闭。旧近似后端保持冻结且行为未改。完整证据见 [transparent-draw-correlation.md](transparent-draw-correlation.md)。
...
逐条Opaque状态显示VS、PS、layout、材质主纹理SRV 0–2和其余材质资源均稳定；仅VS transform/global常量随帧变化，PS全局frame buffer和一个场景公共SRV按游戏ring正常轮换。这排除了selector、material texture、重复draw和command资源悬空。IDA随后给出直接证据：原生non-skinned transform updater `0x266DD0`从`Render.Manager.Views[30].SubViews[12].Camera.ViewMatrix`生成WorldView；我们的hook rendezvous虽记录`View=30/SubView=11`，但`CurrentSubViewIndex=11`是Context调度标签，不是`Render.View.SubViews[]`索引。此前改用`Control.CameraManager.GetActiveCamera()`仍与shader实际的Render camera不一致，必然影响所有正式world实例的temporal/投影稳定性。当前后端改为与原生updater完全相同的View 30/SubView 12 camera。`Clear EventHorizon logs`和四submission窄capture暂留到实机确认不闪后再收尾。

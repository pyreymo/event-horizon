...
internal sealed unsafe class PlayerPreviewHighlighter : IDisposable
{
    ...
    private PlayerHandle? selectedEntityId;
    private long selectedLeaseExpiresAt;
    private HighlightedObject? highlightedObject;

    public void SetSelectedPlayer(PlayerHandle? entityId)
    {
    ...
    public void Update()
    {
        ...
        var gameObject = manager->Objects.GetObjectByEntityId(selectedEntityId.Value.EntityId);
        if (!IsUsableGameObject(gameObject) || (ulong)gameObject->GetGameObjectId() != selectedEntityId.Value.GameObjectId)
        {

...
internal sealed class PlayerPreviewRenderer
{
    ...
    private PlayerHandle? selectedPlayerEntityId;
    ...
    public PlayerPreviewRenderResult Draw(PlayerPreviewSnapshot snapshot, float side, Func<PlayerKeepRuleId, string> getRuleLabel)
    {
        ...
        if (snapshot.Players.Count == 0)
        {
            ...
            {
                PlayerPreviewEmptyReason.NativeHookFailed => Loc.Text("PauseReason.NativeHookFailed"),
        ...
        ImGui.Dummy(size);
        return new(pointedPlayer, selectedPlayer?.Handle);
    }
    ...
    )
    {
        ...
        if (ImGui.IsMouseClicked(ImGuiMouseButton.Left))
        {
            ...
            selectedPlayerEntityId = hoveredPlayer?.Handle;
        }
    }
    ...
    private bool IsSelected(PlayerPreviewEntry player) => selectedPlayerEntityId == player.Handle;
        ...
        try
        {
            ...
            ImGui.TextUnformatted(
                string.Format(
                    ...
                    Loc.Text(
                        player.Allowed switch
                        {
                            true => "Feature.Preview.Allowed",
                            false => "Feature.Preview.Rejected",
                            _ => "Feature.Preview.Unknown",
                        }
                    )
                )
            );

            if (player.Allowed.HasValue)
            {
                ImGui.TextUnformatted(Loc.Text(player.Player.InDrawRange ? "Feature.Preview.InRange" : "Feature.Preview.OutOfRange"));
                if (player.Player.TemporaryReveal)
                    ImGui.TextUnformatted(Loc.Text("Feature.Preview.Revealed"));
            }
...
internal readonly record struct PlayerPreviewRenderResult(PlayerPreviewEntry? PointedPlayer, PlayerHandle? SelectedPlayerEntityId);
...
internal static unsafe class PlayerPreviewWorldArrowRenderer
{
    ...
    {
        return IsUsableGameObject(gameObject)
            && gameObject->EntityId == player.EntityId
            && (ulong)gameObject->GetGameObjectId() == player.Handle.GameObjectId;
    }

...
) : Feature<DtrSettings>(settings, save)
{
    internal static IFeatureDefinition CreateDefinition(
        ...
        new FeatureDefinition<DtrSettings>(
            "dtr",
            "Feature.Name.Dtr",
            store => store.LegacyEnabled("ShowDtrBar", true),
            (settings, save) => new DtrFeature(settings, save, dtrBar, reader, commands, openSettings)
        );

    public override void Enable(FeatureScope scope)
    {
        var bar = scope.Own(
            new DtrBar(dtrBar, Settings, reader.GetStatus, commands.SetEnabled, openSettings, () => commands.Enabled, scope)
        );
        bar.RefreshNow();
    ...
    private sealed class DtrBar : IDisposable
    {
...
internal sealed class DtrSettings : IFeatureSettings
{
    public int Version { get; set; } = 1;
    public bool ShowFrameRateInDtrBar { get; set; } = false;
}

...
internal sealed unsafe class TargetingMarkerController : IDisposable
{
    ...
    private readonly TargetingMarkerSettings configuration;
    ...
    public TargetingMarkerController(
        ...
        TargetingMarkerSettings configuration,
    ...
    )
    {
        ...
        scope.Own(this);
        void Setup(AddonEvent e, AddonArgs args) => scope.Run(() => OnNamePlatePostSetup(e, args));
        void Finalize(AddonEvent e, AddonArgs args) => scope.Run(() => OnNamePlatePreFinalize(e, args));
        void Update(INamePlateUpdateContext context, IReadOnlyList<INamePlateUpdateHandler> handlers) =>
            scope.Run(() => OnNamePlateDataUpdate(context, handlers));
        scope.Defer(() => addonLifecycle.UnregisterListener(AddonEvent.PostSetup, NamePlateAddonName, Setup));
        addonLifecycle.RegisterListener(AddonEvent.PostSetup, NamePlateAddonName, Setup);
        scope.Defer(() => addonLifecycle.UnregisterListener(AddonEvent.PreFinalize, NamePlateAddonName, Finalize));
        addonLifecycle.RegisterListener(AddonEvent.PreFinalize, NamePlateAddonName, Finalize);
        scope.Defer(() => namePlateGui.OnDataUpdate -= Update);
        namePlateGui.OnDataUpdate += Update;
    ...
    {
        return configuration.EnableTargetingMeNamePlateMarker;
    }

    private bool ShouldUseTargetingMeVfx()
    {
        return configuration.EnableTargetingMeVfxMarker;
    }
